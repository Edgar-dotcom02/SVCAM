...
int main(int argc, char** argv)
{
	...
	const float max_dist = config["max_depth_mm"].as<float>();
    
	const std::vector<std::string> stream_requests = {"left=100000", "depth=100000"};
    ...
    while (!stop.load(std::memory_order_relaxed))
    {
		...
		float best_dist = 5.0f; // m, Reset for the current frame
		float best_x = 0.0f;
		float best_y = 0.0f;
		float best_r = 0.0f;
		...
		if (!circles.empty()) 
		{
			...
			if (circle_found_in_frame && best_dist < max_dist) 
			{
				dist_mm = (int)(best_dist * 1000);
				cv::Point2f center(best_x, best_y);

				std::cout << "Distance: " << best_dist << " m" << std::endl;
